...
class ModelModule(LightningModule):
    def __init__(self, cfg):
        ...
        if self.cfg.pretrained_model_path:
            ckpt = torch.load(self.cfg.pretrained_model_path, map_location=lambda storage, loc: storage, weights_only=False)
    ...
    def _step(self, batch, batch_idx, step_type):
        ...
        with torch.no_grad():
            for i in range(batch_size):
                video_feat, _ = self.model.encoder(batch["videos"][i].unsqueeze(0).to(self.device), None)
                audio_feat, _ = self.model.aux_encoder(batch["audios"][i].unsqueeze(0).to(self.device), None)
                audiovisual_feat = self.model.fusion(torch.cat((video_feat, audio_feat), dim=-1))
                audiovisual_feat = audiovisual_feat.squeeze(0)
                nbest_hyps = self.beam_search(audiovisual_feat)
                nbest_hyps = [h.asdict() for h in nbest_hyps[: min(len(nbest_hyps), 1)]]
                ...
                edit_dist = compute_word_level_distance(target, predicted)
                target_len = len(target.split())
                wer = edit_dist / target_len if target_len > 0 else 1.0
                self.log(f"{step_type}_wer", wer, batch_size=1)
...
def get_beam_search_decoder(model, token_list, ctc_weight=0.0, beam_size=40):
    scorers = {
